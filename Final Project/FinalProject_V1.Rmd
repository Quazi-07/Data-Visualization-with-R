...
Data_perform_all %>%
  ...
  gather(columns, points, `Exam_1(in %)`:`Exam_4(in %)`) %>%
  ggplot(aes(x = columns, y = points, group = row_number, color = row_number)) +
...
# transpose 
Data_perform_trans <- as.data.frame(t(Data_perform))
view(Data_perform_trans)

# Rename column
colnames(Data_perform_trans) <- c("Points")

# Rename 1st column which is mising (NA)
Data_perform_trans <- rownames_to_column(Data_perform_trans, var = "Exams")
view(Data_perform_trans)

ggplot(Data_perform_trans, aes(Exams,Points))+ geom_point()
```
